...
type Tab = 'dashboard' | 'create' | 'list' | 'analytics' | 'admin';
...
export default function App() {
  ...
  const fetchAnalytics = async () => {
    if (!user) return;
    try {
      const res = await fetch(`/api/v1/user/analytics?userId=${user.uid}`);
  ...
  useEffect(() => {
    if (activeTab === 'admin' && !profile?.isAdmin && profile?.email !== 'hqluan13081996@gmail.com') {
  ...
  useEffect(() => {
    if (user && profile?.isApproved) {
      ...
      if (activeTab === 'admin' && (profile?.isAdmin || profile?.email === 'hqluan13081996@gmail.com')) fetchAllUsers();
    }
  ...
  const fetchStats = async () => {
    if (!user) return;
    try {
      const response = await fetch(`/api/v1/user/stats?userId=${user.uid}`);
  ...
  const fetchLinks = async () => {
    ...
    setListLoading(true);
    try {
      const response = await fetch(`/api/v1/user/links?userId=${user.uid}`);
      const data = await response.json();
      setLinks(data.sort((a: any, b: any) => b.createdAt?._seconds - a.createdAt?._seconds));
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const fetchAllUsers = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/admin/users?adminId=${user.uid}`);
      const data = await response.json();
      setAllUsers(data);
    } catch (e) {
      console.error(e);
      ...
      <div className="min-h-screen bg-[#FDFCFB] flex flex-col items-center justify-center p-6 font-sans">
        ...
        >
          ...
          <p className="text-gray-500 text-lg mb-10 leading-relaxed">
            Hệ thống quản lý link tiếp thị liên kết thông minh trên tên miền <span className="text-orange-600 font-bold">hotsnew.click</span>
          </p>
  ...
  const isActuallyAdmin = profile?.isAdmin || profile?.email === 'hqluan13081996@gmail.com'; 
  if (!profile?.isApproved && !isActuallyAdmin) {
    return (
      <div className="min-h-screen bg-[#FDFCFB] flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-3xl font-black text-gray-900 mb-4">Đang Chờ Duyệt</h2>
        <p className="text-gray-500 mb-8 max-w-sm">Chào mừng bạn gia nhập cộng đồng <b>hotsnew.click</b>. Tài khoản đang chờ kích hoạt từ Admin.</p>
        <button onClick={logout} className="px-8 py-3 bg-gray-900 text-white rounded-xl font-bold">Đăng xuất</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#F8F9FA] flex">
      ...
      <aside className="w-72 bg-white border-r border-gray-200 hidden lg:flex flex-col p-6 sticky top-0 h-screen">
        <div className="flex items-center gap-3 mb-12">
          ...
          <span className="font-black text-xl tracking-tight">HOTSNEW<span className="text-orange-600">.CLICK</span></span>
        </div>
        ...
        <nav className="flex-1 space-y-2">
          ...
          <SidebarItem icon={List} label="Quản lý Link" active={activeTab === 'list'} onClick={() => setActiveTab('list')} />
          <SidebarItem icon={BarChart3} label="Hiệu quả" active={activeTab === 'analytics'} onClick={() => setActiveTab('analytics')} />
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'dashboard' && (
            ...
            >
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-gray-100">
                <div>
                   <h2 className="text-4xl font-black text-gray-900 mb-3 tracking-tight">Xin chào, {user.displayName?.split(' ')[0]}! 👋</h2>
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-gray-400 font-bold font-mono">domain: hotsnew.click</span>
                   </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { label: 'Tài nguyên Link', value: stats.totalLinks, icon: Zap, color: 'text-orange-600', bg: 'bg-orange-50', footer: 'Cập nhật từ Database' },
                  { label: 'Lượt Clicks', value: stats.totalClicks, icon: MousePointer2, color: 'text-blue-600', bg: 'bg-blue-50', footer: 'Xác thực IP thời gian thực' },
                  { label: 'Trạng thái', value: 'Verified', icon: ShieldCheck, color: 'text-green-600', bg: 'bg-green-50', footer: 'Acc: Premium Access' },
                ].map((s, i) => (
                  <div key={i} className="group bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all">
                    ...
                    <div className="pt-4 border-t border-gray-50 flex items-center gap-2 text-[10px] font-bold text-gray-400 italic">
                       <Activity size={12} className="text-green-500" /> {s.footer}
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="lg:col-span-3 bg-gray-900 rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl flex flex-col justify-end min-h-[350px]">
                  ...
                  <div className="relative z-10 space-y-6">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-2">
                       <VideoIcon size={12} className="text-orange-500" /> Video Landing Page
                    </div>
                    <h3 className="text-3xl font-black leading-tight max-w-sm font-sans tracking-tight">Tối ưu tỷ lệ Click với Landing mờ ảo.</h3>
                    <p className="text-gray-400 max-w-md font-medium text-sm leading-relaxed">Kết hợp Cloudinary Stream và Shopee URLs để tạo ra trải nghiệm chuyển hướng mượt mà, đầy sức hút.</p>
                    <button 
                      onClick={() => setActiveTab('create')}
                    ...
                    >
                      Bắt đầu chiến dịch <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                    </button>
                  </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex-1">
                     <div className="flex items-center justify-between mb-8">
                        <h4 className="text-sm font-black text-gray-900 uppercase tracking-widest">Quy trình Hoạt động</h4>
                        <div className="w-6 h-6 bg-gray-50 rounded-lg flex items-center justify-center text-gray-300"><ChevronRight size={14}/></div>
                     </div>
                     <div className="space-y-6">
                        {[
                          { step: '01', title: 'Upload Video', desc: 'Lưu trữ trực tiếp trên Cloudinary SDK.', color: 'text-orange-500' },
                          { step: '02', title: 'Dán Link Shopee', desc: 'Xác thực URL tự động từ hệ thống.', color: 'text-blue-500' },
                          { step: '03', title: 'Tạo Mã Ngắn', desc: 'Redirect 302 bảo mật cao.', color: 'text-green-500' },
                        ].map((item, idx) => (
                          <div key={idx} className="flex gap-4">
                             <div className={`text-xs font-black ${item.color} font-mono mt-1`}>{item.step}</div>
                             <div>
                  ...
                  <div className="bg-gradient-to-br from-blue-600 to-blue-800 p-8 rounded-[2.5rem] text-white shadow-lg">
                     <div className="flex items-center gap-3 mb-4">
                        ...
                        <h4 className="text-xs font-black uppercase tracking-widest">News & Ops</h4>
                     </div>
                     <p className="text-sm font-bold leading-snug mb-4">Module Phân tích Luồng (Analytics v2.0) đã được kích hoạt thành công trên hệ thống.</p>
                     <button 
              ...
              {links.length > 0 && (
                <div className="space-y-6">
                   <div className="flex items-center justify-between px-2">
                      <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest">Tài nguyên gần đây</h3>
                      <button onClick={() => setActiveTab('list')} className="text-[10px] font-black text-orange-600 uppercase tracking-widest hover:underline">Xem tất cả</button>
                   </div>
                      ...
                      {links.slice(0, 3).map((l, idx) => (
                        <div key={idx} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-all">
                           <div className="w-12 h-12 bg-gray-50 rounded-xl overflow-hidden shrink-0">
                              {l.videoUrl ? (
                                <div className="w-full h-full bg-gray-900 flex items-center justify-center text-white"><VideoIcon size={18} /></div>
                              ) : l.customImageUrl ? (
                                <img src={l.customImageUrl} className="w-full h-full object-cover" />
                              ) : <ImageIcon size={20} className="text-gray-200" />}
                           </div>
                           <div className="min-w-0 flex-1">
                              <p className="text-xs font-black text-gray-900 truncate">{l.customTitle || 'Không tên'}</p>
                              <p className="text-[10px] font-bold text-gray-400 font-mono tracking-tighter truncate">/s/{l.shortCode}</p>
                           </div>
                           <button onClick={() => {
                             navigator.clipboard.writeText(`https://hotsnew.click/s/${l.shortCode}`);
                             setCopiedId(l.id || null);
                             setTimeout(() => setCopiedId(null), 2000);
                           }} className="p-2 hover:bg-orange-50 hover:text-orange-600 rounded-lg transition-all text-gray-300">
                              {copiedId === l.id ? <Check size={16} className="text-green-500" /> : <Copy size={16} />}
                           </button>
                        </div>
                      ))}
                   </div>
          ...
          {activeTab === 'create' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} key="create">
              <header className="mb-12">
                <h2 className="text-3xl font-black text-gray-900 mb-2">Tạo Link Transformer</h2>
                <p className="text-gray-500 font-medium italic">Tùy chỉnh tiêu đề, hình ảnh và video cho hotsnew.click</p>
              </header>
                  ...
                  <form onSubmit={handleConvert} className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm space-y-6">
                    <div>
                      ...
                      <div className="grid grid-cols-1 gap-5">
                        ...
                        <input 
                          type="file" accept="video/*" ref={videoInputRef} onChange={handleVideoUpload} className="hidden"
                        />
                ...
                <div className="space-y-6">
                  ...
                  <div className={cn(
                    ...
                    <div className="flex items-center justify-between mb-6">
                       <span className="text-[10px] font-black uppercase tracking-widest text-orange-600">Short Link ID: {result?.shortCode || '########'}</span>
                       <button onClick={() => copyToClipboard(result?.converted_url, 'res')} className="p-2 hover:bg-orange-50 rounded-lg text-orange-600 transition-all">
                    ...
                    <div className="bg-gray-50 p-5 rounded-2xl font-bold text-xs truncate mb-8 text-gray-500">
                       {result?.converted_url || 'https://hotsnew.click/s/########'}
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4">
                      <button 
                        onClick={() => copyToClipboard(result?.converted_url, 'res')}
                        disabled={!result}
              ...
              <div className="bg-white rounded-[3rem] border border-gray-100 overflow-hidden shadow-sm">
                ...
                <div className="divide-y divide-gray-100">
                  {adminLoading ? (
                    ...
                    <div key={u.uid} className="p-6 flex items-center justify-between gap-6 hover:bg-gray-50 transition-all">
                       <div className="flex items-center gap-4">
                          <img src={u.photoURL || 'https://via.placeholder.com/40'} className="w-12 h-12 rounded-full ring-2 ring-white shadow-md" />
                          <div>
                             <p className="font-bold text-gray-900">{u.displayName || 'Unnamed User'}</p>
                ...
                {listLoading ? (
                   ...
                   <div key={l.id} className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col sm:flex-row items-center gap-6 group hover:shadow-xl transition-all">
                      ...
                      <div className="flex-1 min-w-0 pr-4">
                        <h4 className="font-bold text-gray-900 truncate mb-1">{l.customTitle || 'Untitled link'}</h4>
                        <p className="text-xs text-gray-400 font-medium truncate mb-2">{l.customDescription || 'No description provided'}</p>
                        <div className="flex items-center gap-3">
                           <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg">{l.shortCode}</span>
                           <span className="text-[10px] font-medium text-gray-400 uppercase tracking-tighter">
                             {l.createdAt && formatDistanceToNow(new Date(l.createdAt._seconds * 1000))} ago
                           </span>
                        </div>
                      </div>
                      <div className="flex gap-2">
                         <button onClick={() => copyToClipboard(`https://hotsnew.click/s/${l.shortCode}`, l.id || '')} className="px-6 py-3 bg-gray-900 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all">
                           {copiedId === l.id ? 'DONE' : 'COPY'}
                         </button>
                         <a href={l.originalUrl} target="_blank" className="p-3 bg-gray-50 text-gray-400 rounded-xl hover:text-orange-600 hover:bg-orange-50 transition-all"><ExternalLink size={18} /></a>
                      </div>
                   </div>
                ))}
              </div>
          ...
          {activeTab === 'analytics' && (
            ...
            >
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    { label: 'Tổng Lượt Click', value: analyticsData.history.reduce((a, b) => a + b.clicks, 0).toLocaleString(), icon: MousePointer2, color: 'text-orange-500', bg: 'bg-orange-50' },
                    { label: 'Link Hoạt Động', value: links.length, icon: Activity, color: 'text-blue-500', bg: 'bg-blue-50' },
                    { label: 'Tăng Trưởng (30d)', value: '+12.5%', icon: TrendingUp, color: 'text-green-500', bg: 'bg-green-50' },
                  ].map((stat, idx) => (
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
                     ...
                     <div className="space-y-4">
                        {analyticsData.topLinks.length > 0 ? analyticsData.topLinks.map((item, idx) => (
                           <div key={idx} className="flex items-center gap-4 p-4 hover:bg-gray-50 rounded-2xl transition-all group">
                              ...
                              <div className="w-24 bg-gray-100 h-1.5 rounded-full overflow-hidden">
                                 <div 
                                    ...
                                    style={{ width: `${(item.clicks / Math.max(...analyticsData.topLinks.map(l => l.clicks))) * 100}%` }}
                                 />
                              </div>
                           </div>
                        )) : (
